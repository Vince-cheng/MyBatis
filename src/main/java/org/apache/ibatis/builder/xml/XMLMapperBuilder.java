...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  private void parsePendingCacheRefs() {
    ...
    synchronized (incompleteCacheRefs) {
      ...
      while (iter.hasNext()) {
        try {
          // 尝试解析 <cache-ref> 节点，若解析失败，则抛出 IncompleteElementException，此时下面的删除操作不会被执行
  ...
  private void cacheRefElement(XNode context) {
    if (context != null) {
      ...
      try {
        ...
        // 捕捉 IncompleteElementException 异常，并将 cacheRefResolver 存入到 Configuration 的 incompleteCacheRefs 集合中
